function classifyActivities( measF, clasF, classifiers, folds, nrrep, statDisp, plot)
%
...
%
    measF = convertStringsToChars(measF);
    resultFileName = strcat(measF(1:end-9), "results.txt");
    ...
    fprintf(resultFile, 'Size: %dx%d; EVENT: %s; NON-EVENT: %s\n',size(meas,1), size(meas,2), clas1, clas2);
    fclose(resultFile);
...
%             set(findall(gcf,'-property','FontSize'),'FontSize',fSize)
    ...
    classifyMeasurements(resultFileName, meas, clas, clas1, clas2, classifiers, folds, nrrep, plot);
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function  classifyMeasurements(resultFileName, meas, clas, clas1, clas2, classifiers, folds, nrrep, plot)
%
% The function that performs the actual classification
%   resultFileName - file name to write the results in
...
%   nrepp - the number of averall repetitions
    ...
    resultFile = fopen(resultFileName, "at");  % append, as we already opened and wrote to it once
...
%                 %figure;
%                 %plot (aucX,aucY);
%                 [res] = displayResults('All records-Test  ', TP, FN, FP, TN, auc);
%                 fprintf(res);
%                 fprintf(resultFile, res);
% 
%                 [res] = displayResults('All records-Learn ', TP, FN, FP, TN, auc1);
%                 fprintf(res);
%                 fprintf(resultFile, res);
...
%                   %figure;
%                   %plot (aucX,aucY);
%                   [res] = displayResults('20%% holdout-Test  Single ', TP, FN, FP, TN, auc);
%                   fprintf(res);
%                   fprintf(resultFile, res);
% 
%                   [res] = displayResults('20%% holdout-Learn Single ', TP, FN, FP, TN, auc1);
%                   fprintf(res);
%                   fprintf(resultFile, res);
% 
%                 elseif myidx==nrrep
...
%                   auc2=auc2/nrrep;
%                   auc12=auc12/nrrep;
%                   [res] = displayResults('  20%% holdout-Test  Nrep ', TP2, FN2, FP2, TN2, auc2);
%                   fprintf(res);
%                   fprintf(resultFile, res);
% 
%                   [res] = displayResults('  20%% holdout-Learn Nrep ', TP2, FN2, FP2, TN2, auc12);
%                   fprintf(res);
%                   fprintf(resultFile, res);
%                 end
%          end
...
%                    %figure;
%                    %plot(aucX,aucY);
%                    [res] = displayResults('Cross-valid-Test  Single ', TP, FN, FP, TN, auc);
%                    fprintf(res);
%                    fprintf(resultFile, res);
% 
%                    [res] = displayResults('Cross-valid-Learn Single ', TP, FN, FP, TN, mean(auc1));
%                    fprintf(res);
%                    fprintf(resultFile, res);
                   continue
                ...
                %%
                   ...
                   auc2=auc2/nrrep;
                   auc12=auc12/nrrep;
                   [res] = displayResults('  Cross-valid-Test  Nrep ', TP2, FN2, FP2, TN2, auc2);
                   fprintf(res);
                   fprintf(resultFile, res);

%                    [res] = displayResults('  Cross-valid-Learn Nrep ', TP2, FN2, FP2, TN2, auc12);
%                    fprintf(res);
%                    fprintf(resultFile, res);
                end
         end
...
%                 %figure;
%                 %plot (aucX,aucY);
%                 [res] = displayResults('Leave one o-Test  ', TP, FN, FP, TN,auc);
%                 fprintf(res);
%                 fprintf(resultFile, res);
% 
%                 [res] = displayResults('Leave one o-Learn ', TP, FN, FP, TN, mean(aucAgg));
%                 fprintf(res);
%                 fprintf(resultFile, res);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [res] = displayResults(aText, TP, FN, FP, TN, AUC)
%
...
%
    ...
    if (short==0)
        ...
        res = sprintf('  %s   TP=%3d FN=%3d\n',aText, TP, FN);
        res = append(res, ...
            sprintf('                   FP=%3d TN=%3d\n', FP, TN));
        res = append(res, ...
            sprintf('                   Se: %6.2f Sp: %6.2f\n',TP*100.0/(TP+FN), TN*100.0/(TN+FP)));

    elseif (short==1)
        if (nargin==5)
            % fprintf('   %s TP=%3d FN=%3d; FP=%3d TN=%3d; Se: %6.2f Sp: %6.2f\n',aText,TP,FN,FP,TN,TP*100.0/(TP+FN), TN*100.0/(TN+FP));
            res = ...
                sprintf('   %s TP=%3d FN=%3d; FP=%3d TN=%3d; Se: %6.2f Sp: %6.2f\n',aText,TP,FN,FP,TN,TP*100.0/(TP+FN), TN*100.0/(TN+FP));
        else
            ...
            res = ...
                sprintf('   %s TP=%3d FN=%3d; FP=%3d TN=%3d; Se: %6.2f Sp: %6.2f CA: %6.2f AUC: %6.2f\n',aText,TP,FN,FP,TN,TP*100.0/(TP+FN),TN*100.0/(TN+FP), (TP+TN)*100.0/(TP+FN+FP+TN), AUC*100);
        end
    else
        % fprintf('   %s Se: %6.2f Sp: %6.2f\n',aText, TP*100.0/(TP+FN), TN*100.0/(TN+FP));
        res = sprintf('   %s Se: %6.2f Sp: %6.2f\n',aText, TP*100.0/(TP+FN), TN*100.0/(TN+FP));
    end    
end
